...
class M2:
    ...
    def total_loss(self, mu_l, sig_l, z_l, y_l, mu_u, sig_u, z_u, y_u):
        ...
        cross_entropy = tf.reduce_mean(tf.losses.softmax_cross_entropy(onehot_labels=self.label, 
                                                                       logits=self.pred_l))
...
# train M1
epochs = tqdm.trange(total_epoch, desc='M1-Loss')
for e in epochs:
    total_loss = 0
...
# train M2
epochs = tqdm.trange(total_epoch*10, desc='M2-Loss')
for e in epochs:
    total_loss = 0
    for i in range(int(mnist.train.num_examples//batch_size)):
        ...
        loss, _ = sess.run([m2.loss, m2.opt], feed_dict={m2.x_l:x_l, 
                                                         ...
                                                         m2.lr:lr*1e-2})
        total_loss += loss
    epochs.set_description('M2-Loss=%g' % total_loss)
